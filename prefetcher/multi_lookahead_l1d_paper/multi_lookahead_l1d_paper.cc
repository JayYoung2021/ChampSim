...
public:
      ...
      : Super(size, num_ways, debug_level), blocks_in_zone(blocks_in_zone), queue_size(queue_size)
  {
    if (this->debug_level >= 1)
      cerr << "AccessMapTable::AccessMapTable(size=" << size << ", blocks_in_zone=" << blocks_in_zone << ", queue_size=" << queue_size
           << ", debug_level=" << debug_level << ", num_ways=" << num_ways << ")" << endl;
  }
  ...
  void set_state(uint64_t block_number, State new_state, int new_fill_level = 0)
  {
    if (this->debug_level >= 2)
      cerr << "AccessMapTable::set_state(block_number=0x" << hex << block_number << ", new_state=" << state_char[new_state]
           << ", new_fill_level=" << new_fill_level << ")" << dec << endl;

    // if (new_state != State::PREFTCH)
    //     assert(new_fill_level == 0);
    // else
    //     assert(new_fill_level == static_cast<int>(DeprecatedFillLevel::FILL_L1) || new_fill_level == static_cast<int>(DeprecatedFillLevel::FILL_L2) || new_fill_level == static_cast<int>(DeprecatedFillLevel::FILL_LLC));
    ...
    if (this->debug_level >= 2) {
      cerr << "[AccessMapTable::set_state] zone_number=0x" << hex << zone_number << dec << ", zone_offset=" << setw(2) << zone_offset
           << ": state transition from " << state_char[old_state] << " to " << state_char[new_state] << endl;
      if (old_state != new_state || old_fill_level != new_fill_level) {
        cerr << "[AccessMapTable::set_state] old_access_map=" << map_to_string(old_access_map, old_prefetch_map) << endl;
        cerr << "[AccessMapTable::set_state] new_access_map=" << map_to_string(access_map, prefetch_map) << endl;
      }
    }
  }
...
{
public:
  MLOP(int blocks_in_zone, int amt_size, const int PF_DEGREE, const int NUM_UPDATES, const double L1D_THRESH, const double L2C_THRESH, const double LLC_THRESH,
       int debug_level)
      : PF_DEGREE(PF_DEGREE), NUM_UPDATES(NUM_UPDATES), L1D_THRESH(L1D_THRESH * NUM_UPDATES), L2C_THRESH(L2C_THRESH * NUM_UPDATES),
        LLC_THRESH(LLC_THRESH * NUM_UPDATES), ORIGIN(blocks_in_zone - 1), MAX_OFFSET(blocks_in_zone - 1), NUM_OFFSETS(2 * blocks_in_zone - 1),
        blocks_in_zone(blocks_in_zone), access_map_table(amt_size, blocks_in_zone, PF_DEGREE - 1, debug_level), pf_offset(PF_DEGREE, vector<int>()),
        offset_scores(PF_DEGREE, vector<int>(2 * blocks_in_zone - 1, 0)), pf_level(PF_DEGREE, 0), debug_level(debug_level)
  {
    if (this->debug_level >= 1)
      cerr << "MLOP::MLOP(blocks_in_zone=" << blocks_in_zone << ", amt_size=" << amt_size << ", PF_DEGREE=" << PF_DEGREE << ", NUM_UPDATES=" << NUM_UPDATES
           << ", L1D_THRESH=" << L1D_THRESH << ", L2C_THRESH=" << L2C_THRESH << ", LLC_THRESH=" << LLC_THRESH << ", debug_level=" << debug_level << ")" << endl;
  ...
  void access(uint64_t block_number)
  {
    ...
    if (this->debug_level >= 2)
      cerr << "[MLOP::access] zone_number=0x" << hex << zone_number << dec << ", zone_offset=" << zone_offset << endl;
    ...
    if (this->update_cnt == NUM_UPDATES) {
      ...
      for (int d = PF_DEGREE - 1; d >= 0; d -= 1) {
        ...
        if (max_scores[d] >= L1D_THRESH)
          fill_level = static_cast<int>(DeprecatedFillLevel::FILL_L1);
        else if (max_scores[d] >= L2C_THRESH)
          fill_level = static_cast<int>(DeprecatedFillLevel::FILL_L2);
        else if (max_scores[d] >= LLC_THRESH)
          fill_level = static_cast<int>(DeprecatedFillLevel::FILL_LLC);
        else
          continue;
      ...
      if (this->debug_level >= 1) {
        for (int d = 0; d < PF_DEGREE; d += 1) {
          ...
          if (this->pf_level[d] == 0)
            cerr << "No";
          if (this->pf_level[d] == static_cast<int>(DeprecatedFillLevel::FILL_L1))
            cerr << "L1";
          if (this->pf_level[d] == static_cast<int>(DeprecatedFillLevel::FILL_L2))
            cerr << "L2";
          if (this->pf_level[d] == static_cast<int>(DeprecatedFillLevel::FILL_LLC))
            cerr << "L3";
   ...
   */
  void prefetch(CACHE* cache, uint64_t block_number, const int degree)
  {
    if (this->debug_level >= 2) {
      cerr << "MLOP::prefetch(cache=" << cache->NAME << "-" << cache->cpu << ", block_number=0x" << hex << block_number << dec << ")" << endl;
    }
    int pf_issued = 0;
    uint64_t zone_number = block_number / this->blocks_in_zone;
    int zone_offset = block_number % this->blocks_in_zone;
    ...
    for (int d = 0; d < degree; d += 1) {
      for (auto& cur_pf_offset : this->pf_offset[d]) {
        ...
        if (access_map[offset_to_prefetch] == State::ACCESS)
          continue;
        if (access_map[offset_to_prefetch] == State::PREFTCH && prefetch_map[offset_to_prefetch] <= this->pf_level[d])
          continue;

        if (this->is_inside_zone(offset_to_prefetch) && cache->get_pq_occupancy().back() < cache->get_pq_size().back()
            && cache->get_pq_occupancy().back() + cache->get_mshr_occupancy() < cache->get_mshr_size() - 1) {
          ...
          int ok = cache->prefetch_line(pf_addr, this->pf_level[d], 0);
          // assert(ok == 1);
  ...
  void mark(uint64_t block_number, State state, int fill_level = 0) { this->access_map_table.set_state(block_number, state, fill_level); }
...
void CACHE::prefetcher_initialize()
{
  ...
  L1D_PREF::prefetchers = vector<L1D_PREF::MLOP>(
      NUM_CPUS, L1D_PREF::MLOP(BLOCKS_IN_ZONE, AMT_SIZE, PREFETCH_DEGREE, NUM_UPDATES, L1D_THRESH, L2C_THRESH, LLC_THRESH, L1D_PREF::DEBUG_LEVEL));
}
...
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, bool useful_prefetch, uint8_t type, uint32_t metadata_in)
{
  ...
  if (cache_hit == 1) {
    ...
    if (block[NUM_WAY * set + way].prefetch == 1)
      prefetch_hit = true;
  }
