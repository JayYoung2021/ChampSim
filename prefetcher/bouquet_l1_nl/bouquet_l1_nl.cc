...
float mpkc[NUM_CPUS] = {0};  // Misses per Kilo-cycle
int spec_nl[NUM_CPUS] = {0}; // Speculative Next Line bit
...
{
  const uint64_t pf_address = addr + (1 << LOG2_BLOCK_SIZE);
  const uint64_t current_core_cycle = this->p_cpu->current_cycle;
  ...
  if (num_misses[cpu] == 256) {
    mpkc[cpu] = ((float)num_misses[cpu] / (current_core_cycle - prev_cpu_cycle)) * 1000;
    prev_cpu_cycle = current_core_cycle;
