...
enum struct DeprecatedFillLevel : char { FILL_L1 = 1, FILL_L2 = 2, FILL_LLC = 4, FILL_DRC = 8, FILL_DRAM = 16 };
...
public:
  FilterTable(int size, int debug_level = 0, int num_ways = 16) : Super(size, num_ways, debug_level)
  {
    ...
    if (this->debug_level >= 1)
      cerr << "FilterTable::FilterTable(size=" << size << ", debug_level=" << debug_level << ", num_ways=" << num_ways << ")" << dec << endl;
  }
  ...
  {
    if (this->debug_level >= 2)
      cerr << "FilterTable::insert(region_number=0x" << hex << region_number << ", pc=0x" << pc << ", offset=" << dec << offset << ")" << dec << endl;
    uint64_t key = this->build_key(region_number);
...
public:
  AccumulationTable(int size, int pattern_len, int debug_level = 0, int num_ways = 16) : Super(size, num_ways, debug_level), pattern_len(pattern_len)
  {
    // assert(__builtin_popcount(size) == 1);
    // assert(__builtin_popcount(pattern_len) == 1);
    if (this->debug_level >= 1)
      cerr << "AccumulationTable::AccumulationTable(size=" << size << ", pattern_len=" << pattern_len << ", debug_level=" << debug_level
  ...
  {
    if (this->debug_level >= 2)
      cerr << "AccumulationTable::set_pattern(region_number=0x" << hex << region_number << ", offset=" << dec << offset << ")" << dec << endl;
    uint64_t key = this->build_key(region_number);
    Entry* entry = Super::find(key);
    if (!entry) {
      if (this->debug_level >= 2)
  ...
  {
    if (this->debug_level >= 2)
      cerr << "AccumulationTable::insert(region_number=0x" << hex << region_number << ", pc=0x" << pc << ", offset=" << dec << offset << dec << endl;
    uint64_t key = this->build_key(region_number);
    // assert(!Super::find(key));
...
public:
      ...
      : Super(size, num_ways, debug_level), pattern_len(pattern_len), min_addr_width(min_addr_width), max_addr_width(max_addr_width), pc_width(pc_width)
  {
    ...
    // assert(__builtin_popcount(pattern_len) == 1);
    if (this->debug_level >= 1)
      cerr << "PatternHistoryTable::PatternHistoryTable(size=" << size << ", pattern_len=" << pattern_len << ", min_addr_width=" << min_addr_width
           << ", max_addr_width=" << max_addr_width << ", pc_width=" << pc_width << ", debug_level=" << debug_level << ", num_ways=" << num_ways << ")" << dec
           << endl;
  }
  ...
  {
    if (this->debug_level >= 2)
      cerr << "PatternHistoryTable::insert(pc=0x" << hex << pc << ", address=0x" << address << ", pattern=" << pattern_to_string(pattern) << ")" << dec << endl;
...
public:
  PrefetchStreamer(int size, int pattern_len, int debug_level = 0, int num_ways = 16) : Super(size, num_ways, debug_level), pattern_len(pattern_len)
  {
    if (this->debug_level >= 1)
      cerr << "PrefetchStreamer::PrefetchStreamer(size=" << size << ", pattern_len=" << pattern_len << ", debug_level=" << debug_level
           << ", num_ways=" << num_ways << ")" << dec << endl;
  }
  ...
  {
    if (this->debug_level >= 2)
      cerr << "PrefetchStreamer::insert(region_number=0x" << hex << region_number << ", pattern=" << pattern_to_string(pattern) << ")" << dec << endl;
    uint64_t key = this->build_key(region_number);
    Super::insert(key, {pattern});
    Super::set_mru(key);
  }

  int prefetch(CACHE* cache, uint64_t block_address, const int degree)
  {
    if (this->debug_level >= 2) {
      cerr << "PrefetchStreamer::prefetch(cache=" << cache->NAME << ", block_address=0x" << hex << block_address << ")" << dec << endl;
      cerr << "[PrefetchStreamer::prefetch] " << cache->get_pq_occupancy().back() << "/" << cache->get_pq_size().back() << " PQ entries occupied." << dec
           << endl;
    ...
    for (int d = 1; d < degree; d += 1) {
      ...
      for (int sgn = +1; sgn >= -1; sgn -= 2) {
        ...
        if (0 <= pf_offset && pf_offset < this->pattern_len && pattern[pf_offset] > 0) {
          ...
          if (cache->get_pq_occupancy().back() + cache->get_mshr_occupancy() < cache->get_mshr_size() - 1
              && cache->get_pq_occupancy().back() < cache->get_pq_size().back()) {
            int ok = cache->prefetch_line(pf_address, pattern[pf_offset], 0);
            // assert(ok == 1);
            pf_issued += 1;
            pattern[pf_offset] = 0;
          } else {
...
{
public:
  Bingo(int pattern_len, int min_addr_width, int max_addr_width, int pc_width, int filter_table_size, int accumulation_table_size, int pht_size, int pht_ways,
        int pf_streamer_size, int debug_level = 0)
      : pattern_len(pattern_len), filter_table(filter_table_size, debug_level), accumulation_table(accumulation_table_size, pattern_len, debug_level),
        pht(pht_size, pattern_len, min_addr_width, max_addr_width, pc_width, debug_level, pht_ways), pf_streamer(pf_streamer_size, pattern_len, debug_level),
        debug_level(debug_level)
  {
    if (this->debug_level >= 1)
      cerr << "Bingo::Bingo(pattern_len=" << pattern_len << ", min_addr_width=" << min_addr_width << ", max_addr_width=" << max_addr_width
           << ", pc_width=" << pc_width << ", filter_table_size=" << filter_table_size << ", accumulation_table_size=" << accumulation_table_size
           << ", pht_size=" << pht_size << ", pht_ways=" << pht_ways << ", pf_streamer_size=" << pf_streamer_size << ", debug_level=" << debug_level << ")"
           << endl;
  }
  ...
  int prefetch(CACHE* cache, uint64_t block_number, const int degree)
  {
    if (this->debug_level >= 2)
      cerr << "Bingo::prefetch(cache=" << cache->NAME << ", block_number=" << hex << block_number << ")" << dec << endl;
    int pf_issued = this->pf_streamer.prefetch(cache, block_number, degree);
    if (this->debug_level >= 2)
  ...
  void print_stats()
  {
    ...
    double l1_pref_per_region = 1.0 * this->pref_level_cnt[static_cast<int>(DeprecatedFillLevel::FILL_L1)] / this->region_pref_cnt;
    double l2_pref_per_region = 1.0 * this->pref_level_cnt[static_cast<int>(DeprecatedFillLevel::FILL_L2)] / this->region_pref_cnt;
    double l3_pref_per_region = 1.0 * this->pref_level_cnt[static_cast<int>(DeprecatedFillLevel::FILL_LLC)] / this->region_pref_cnt;
...
private:
  ...
  vector<int> vote(const vector<vector<bool>>& x)
  {
    ...
    for (int i = 0; i < n; i += 1)
      ...
      for (int i = 0; i < this->pattern_len; i += 1) {
        ...
        double p = 1.0 * cnt / n;
        if (p >= L1D_THRESH)
          res[i] = static_cast<int>(DeprecatedFillLevel::FILL_L1);
        else if (p >= L2C_THRESH)
          res[i] = static_cast<int>(DeprecatedFillLevel::FILL_L2);
        else if (p >= LLC_THRESH)
          res[i] = static_cast<int>(DeprecatedFillLevel::FILL_LLC);
        else
          res[i] = 0;
        if (res[i] != 0)
          pf_flag = true;
  ...
  const int PC_ADDRESS_FILL_LEVEL = static_cast<int>(DeprecatedFillLevel::FILL_L1);
  /*======================*/
...
void CACHE::prefetcher_initialize()
{
  ...
  L1D_PREF::prefetchers = vector<L1D_PREF::Bingo>(NUM_CPUS, L1D_PREF::Bingo(REGION_SIZE >> LOG2_BLOCK_SIZE, MIN_ADDR_WIDTH, MAX_ADDR_WIDTH, PC_WIDTH, FT_SIZE,
...
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, bool useful_prefetch, uint8_t type, uint32_t metadata_in)
{
  if (L1D_PREF::DEBUG_LEVEL >= 2) {
    cerr << "CACHE::l1d_prefetcher_operate(addr=0x" << hex << addr << ", ip=0x" << ip << ", cache_hit=" << dec << (int)cache_hit << ", type=" << (int)type
         << ")" << dec << endl;
    cerr << "[CACHE::l1d_prefetcher_operate] CACHE{core=" << this->cpu << ", NAME=" << this->NAME << "}" << dec << endl;
  }
  ...
  /* issue prefetches */
  L1D_PREF::prefetchers[cpu].prefetch(this, block_number, degrees[::private_data[this].degree_index]);
...
uint32_t CACHE::prefetcher_cache_fill(uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in)
{
  ...
  if (this->block[NUM_WAY * set + way].valid == 0)
